 ...
 */
@Slf4j
public abstract class CommonInstanceHealthcheckAction<E extends DeployedExecutionObjectInfo, S extends Enum<S>,
    ...
    protected StateData<S, E> executeImpl(InstanceActionContext<T> context, StateData<S, E> currentState) {
        val healthcheckSpec = healthcheck(context.getInstanceSpec());
        try (val checker = ExecutorUtils.createChecker(context, currentState.getData(), healthcheckSpec)) {
            ...
            if (stop.get()) {
                ...
                return StateData.from(currentState, interruptedState());
            }
        ...
        return StateData.errorFrom(currentState, defaultErrorState(), "Node is unhealthy");
    }
    ...
    protected abstract S interruptedState();
