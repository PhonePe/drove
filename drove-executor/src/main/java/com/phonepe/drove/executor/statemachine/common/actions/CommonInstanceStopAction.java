 ...
 */
@Slf4j
public abstract class CommonInstanceStopAction<E extends DeployedExecutionObjectInfo, S extends Enum<S>, T extends DeploymentUnitSpec>
        extends CommonContainerCleanupAction<E, S, T> {


    @Override
    protected StateData<S, E> preRemoveAction(InstanceActionContext<T> context, StateData<S, E> currentState) {
    ...
    @Override
    protected final S stoppedState() {
        return defaultErrorState();
    }

    protected abstract PreShutdownSpec preShutdownSpec(T spec);

    private void handlePreShutdown(
            final InstanceActionContext<T> context,
            final StateData<S, E> currentState) {
        val instanceSpec = context.getInstanceSpec();
        val preShutdown = Objects.requireNonNullElse(preShutdownSpec(instanceSpec), PreShutdownSpec.DEFAULT);
    ...
    private void executeHttpShutdownHook(
        ...
        final var port = instancePort(currentState, httpSpec);
        if (null == port) {
    ...
    private static <E extends DeployedExecutionObjectInfo, S extends Enum<S>> InstancePort instancePort(
            StateData<S, E> currentState,
            HTTPCheckModeSpec httpSpec) {
        val instanceInfo = currentState.getData();
        return instanceInfo.accept(new DeployedExecutorInstanceInfoVisitor<InstancePort>() {
            @Override
            public InstancePort visit(ExecutorInstanceInfo applicationInstanceInfo) {
                return applicationInstanceInfo.getLocalInfo()
                        .getPorts()
            ...
            public InstancePort visit(ExecutorLocalServiceInstanceInfo localServiceInstanceInfo) {
                return localServiceInstanceInfo.getLocalInfo()
                        .getPorts()
                        .get(httpSpec.getPortName());
            }
        });
