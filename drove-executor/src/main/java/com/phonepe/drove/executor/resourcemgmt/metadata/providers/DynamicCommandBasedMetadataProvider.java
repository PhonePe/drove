...
public class DynamicCommandBasedMetadataProvider implements MetadataProvider {
    public static final String COMMAND_RERUN_TIME_TAKEN = "reruntimetaken";
    public static final String COMMAND_RERUN_TIME_WINDOW = "reruntimewindow";
    ...
    public static class DynamicCommandBasedMetadataProviderFactory implements MetadataProviderFactory<DynamicCommandBasedMetadataProviderConfig, DynamicCommandBasedMetadataProvider> {
        ...
        @Override
        public DynamicCommandBasedMetadataProvider create(
                MetricRegistry metricRegistry,
                DynamicCommandBasedMetadataProviderConfig config) {
    ...
    @Inject
    public DynamicCommandBasedMetadataProvider(
            final MetricRegistry metricsRegistry,
            final DynamicCommandBasedMetadataProviderConfig dynamicCommandBasedMetadataProviderConfig) {
    ...
    public void start(String name) {
        commandRerunTimeTaken = namespacedMetricTimer(name, COMMAND_RERUN_TIME_TAKEN);
        commandRerunTimeWindow = namespacedMetricTimer(name, COMMAND_RERUN_TIME_WINDOW);
    ...
    private void refreshDynamicCommands(Date date) {

        if (System.currentTimeMillis() - lastDynamicCommandSuccessfulEpochInMillis < configRefreshInSecs * 1000L) {
            log.warn("Skipping dynamic command refresh as last successful execution was within the configured " +
                             "interval");
            return;
        }
    ...
    // Suppress "Command injection" warning as we are executing commands that are configured and controlled by the user.
    private static Map.Entry<String, String> commandExecutor(Map.Entry<String, CommandBasedConfig> cmdEntry) {
        ...
        try {
            ...
            log.error("Command returned errored: '{}, exit code: {}, error: {} ignoring the command",
                      cmdEntry.getKey(), exitCode, error);
        }
        catch (Exception e) {
            log.error("Unable to execute command: '{}', ignoring the command", cmdEntry.getKey());
