...
class LocalServiceInstanceStopActionTest extends AbstractTestBase {
    @Test
    void testStopProper() {
        val spec = testLocalServiceInstanceSpec();
        val action = new LocalServiceInstanceStopAction();
        ...
        val startAction = new LocalServiceInstanceRunAction(new ResourceConfig(), ExecutorOptions.DEFAULT,
                                                           CommonTestUtils.httpCaller(),
                                                           MAPPER,
                                                           SharedMetricRegistries.getOrCreate("test"),
                                                           resourceManager);
        val state = startAction.execute(context,
                                        StateData.create(LocalServiceInstanceState.PROVISIONING, createExecutorLocaLServiceInstanceInfo(spec, 8080)));
        assertEquals(LocalServiceInstanceState.DEPROVISIONING,
                     action.execute(context, StateData.from(state, LocalServiceInstanceState.HEALTHY)).getState());
    }
    ...
    void testStopInvalidContainer() {
        ...
        assertEquals(LocalServiceInstanceState.DEPROVISIONING,
                     action.execute(context, StateData.create(LocalServiceInstanceState.HEALTHY, createExecutorLocaLServiceInstanceInfo(spec, 8080)))
                             .getState());
    }
    @Test
