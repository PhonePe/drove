                ...
                return Joiner.on("<br>")
                        .join(cpu.getCores()
                                      ...
                                      .map(entry -> String.format(
                                              "<b>NUMA Node: </b> <span class=\"core-display text-info fw-bold\">%s</span> &nbsp; &nbsp;<b>Allocated Cores:</b> %s",
                                              entry.getKey(),
                                              Joiner.on("&nbsp;")
                                                      .join(entry.getValue()
                                                                    .stream()
                                                                    .map(value -> "<span class=\"core-display text-body\">" + value + "</span>")
                                                                    .toList())))
                                      .toList());
            }
            ...
            public CharSequence visit(MemoryAllocation memory) {
                return Joiner.on("<br")
                        .join(memory.getMemoryInMB()
                                      ...
                                      .stream()
                                      .map(e -> String.format(
                                              "<b>NUMA Node:</b> <span class=\"core-display text-info fw-bold\">%s</span>  &nbsp; &nbsp;<b>Allocated Memory:</b> %d MB",
                                              e.getKey(),
                                              e.getValue()))
                                      .toList());
            }
